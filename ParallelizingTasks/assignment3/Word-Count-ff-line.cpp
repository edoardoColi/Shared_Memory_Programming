...
// Compile using:
// g++ -std=c++20 -I. -I /home/e.coli3/fastflow -Wall -O3 -DNDEBUG -ffast-math -o wc Word-Count-ff-line.cpp -pthread
//
// Execute with:
...
struct Task_t {
    Task_t(std::string line):line(line),local_words(0) {}
    ...
    Task_t *svc(Task_t *in) {
        if (in == nullptr) { // first call
			...
			for (auto fn : filenames) {
				std::ifstream file(fn, std::ios_base::in);
				if (file.is_open()) {
					std::string line;
					while(std::getline(file, line)) {
						if (!line.empty()) {
							Task_t *task = new Task_t(line);
...
struct Worker: ff_node_t<Task_t> {
    Task_t *svc(Task_t *in) {
		...
		char *tmpstr;
		char *token = strtok_r(const_cast<char*>((in->line).c_str()), " \r\n", &tmpstr);
		while(token) {
			++UM[std::string(token)];
			token = strtok_r(NULL, " \r\n", &tmpstr);
			++words;
